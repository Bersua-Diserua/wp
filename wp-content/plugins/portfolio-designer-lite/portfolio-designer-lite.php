...
class PortfolioDesignerLite {
    ...
    protected static $plugin_version = '1.1.2';
    ...
    public function pdl_enqueue_admin_scripts($hook_suffix) {
        ...
        if ( (isset($_GET['page']) && ($_GET['page'] == 'portfolio_lite_settings' || $_GET['page'] == 'about_portfolio_designer_lite' || $_GET['page'] == 'portfolio_lite_welcome' )) ||  $hook_suffix == 'plugins.php') {
            ...
            wp_enqueue_script('portdesignerlite_admincolor_alpha', PORT_LITE_PLUGIN_URL . 'includes/js/wp-color-picker-alpha.min.js', array('wp-color-picker','wpColorPickerL10n'));
    ...
    public function pdl_save_settings() {
        if (isset($_REQUEST['addPortfolioDesigner']) && isset($_REQUEST['page']) && $_REQUEST['page'] === 'portfolio_lite_settings' && isset($_REQUEST['action']) && $_REQUEST['action'] === 'save' && isset($_REQUEST['_wp_portfolio_designer_nonce']) && wp_verify_nonce($_REQUEST['_wp_portfolio_designer_nonce'], '_wp_portfolio_designer_action')) {
            $settings = $_POST;
            ...
            if(isset($settings) && !empty($settings)) {
                foreach($settings as $single_key=>$single_val) {
                    if(is_array($single_val)) {
                        ...
                        if($single_key == 'portfolio_custom_css'){
...
if(!function_exists('pdl_submit_optin')) {
    function pdl_submit_optin() {
        ...
        if(isset($_POST['email'])) {
            $pdl_email = sanitize_email($_POST['email']);
        }
        else {
            $pdl_email = get_option('admin_url');
        }
        if(isset($_POST['type'])) {
            $pdl_submit_type = sanitize_text_field($_POST['type']);
        }
