...
class QLIGG_Backend {
	...
	function add_js() {
		...
		wp_register_style( 'qligg-backend', plugins_url( '/build/backend/css/style.css', QLIGG_PLUGIN_FILE ), array( 'wp-color-picker', 'media-views' ), QLIGG_PLUGIN_VERSION, 'all' );
		wp_localize_script(
		...
		$backend = include_once QLIGG_PLUGIN_DIR . 'build/backend/js/index.asset.php';

		wp_register_script( 'qligg-backend', plugins_url( '/build/backend/js/index.js', QLIGG_PLUGIN_FILE ), $backend['dependencies'], $backend['version'], true );
